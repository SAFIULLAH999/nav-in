          ...
          <div className="hidden md:flex items-center relative flex-1 max-w-md" ref={searchRef}>
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full pl-12 pr-10 py-2 bg-secondary dark:bg-dark-secondary rounded-full outline-none text-sm placeholder:text-text-muted dark:placeholder:text-dark-text-muted text-text dark:text-dark-text"
              />
              {searchQuery && (
                <button
                  onClick={() => {
            ...
            <AnimatePresence>
              {showResults && searchResults && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute top-full left-0 right-0 mt-2 bg-surface dark:bg-dark-surface rounded-xl border border-border dark:border-dark-border shadow-soft z-50 max-h-96 overflow-y-auto"
                >
                  {isSearching ? (
...
function NavIcon({ icon: Icon, label, href }: { icon: any, label: string, href: string }) {
  return (
    <Link
      href={href}
      className="nav-link flex flex-col items-center cursor-pointer p-2 rounded-lg group glow-on-hover"
    >
      <Icon className="w-6 h-6 text-text dark:text-dark-text group-hover:scale-110 group-hover:text-primary transition-all duration-300" />
      <span className="text-xs mt-1 font-medium text-text dark:text-dark-text group-hover:text-primary transition-colors duration-300">{label}</span>
    </Link>
  )
}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="absolute right-0 mt-2 w-56 bg-surface dark:bg-dark-surface rounded-xl border border-border dark:border-dark-border shadow-soft z-50"
          >
  ...
  const renderUserResult = (user: any) => (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-text dark:text-dark-text truncate">{user.name || 'Unknown User'}</p>
        <p className="text-sm text-text-muted dark:text-dark-text-muted truncate">{user.title || 'NavIN User'}</p>
        {user.company && (
          <p className="text-xs text-text-muted dark:text-dark-text-muted truncate">{user.company}</p>
        )}
      </div>
      <div className="text-xs text-primary bg-primary/10 dark:bg-primary/20 px-2 py-1 rounded">
        User
      </div>
    </Link>
  )
